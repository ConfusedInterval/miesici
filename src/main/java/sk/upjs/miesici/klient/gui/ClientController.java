...
public class ClientController {
    ...
    void initialize() {
        ...
        trainingTableView.setOnMouseClicked((MouseEvent event) -> {
    ...
    private void onEdit() {
        if (trainingTableView.getSelectionModel().getSelectedItem() != null) {
            Training selectedCustomer = trainingTableView.getSelectionModel().getSelectedItem();

            TrainingController controller = new TrainingController();
            controller.setTraining(selectedCustomer);
            controller.setCustomer(customer);
            showTraining(controller);

            // refresh table
            exercisesModel = FXCollections.observableArrayList(exerciseDao.getAllByTrainingId(selectedCustomer.getId()));
            trainingTableView.setItems(FXCollections.observableArrayList(trainingsModel));
        }
    }
    ...
    @FXML
    void editTrainingClick(ActionEvent event) {
        Training training = trainingTableView.getSelectionModel().getSelectedItem();
        if (training == null) {
            return;
        }
